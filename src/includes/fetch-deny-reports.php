...
if (isset($_POST["query"])) {
    ...
    if ($_POST["query"] != '') {
        ...
        users.first_name, users.last_name, rescue.animal_image, rescue.deny_reason
        FROM rescue
        ...
        users.first_name, users.last_name, animals.animal_status, rescue.animal_image, rescue.deny_reason
        FROM rescue
        INNER JOIN animals ON rescue.animal_id = animals.animal_id
        INNER JOIN users ON rescue.user_id = users.user_id
        WHERE animals.animal_status IN ('Denied')
    ...
    for ($count = 0; $count < count($page_array); $count++) {
        if ($page == $page_array[$count]) {
            ...
            if ($previous_id > 0) {
                $previous_link = '<li class="page-item"><a class="page-link" href="javascript:load_data_deny(`' . $_POST["query"] . '`, ' . $previous_id . ')"><</a></li>';
            } else {
                $previous_link = '
            ...
            if ($next_id > $total_links) {
                ...
                $next_link = '
                <li class="page-item"><a class="page-link" href="javascript:load_data_deny(`' . $_POST["query"] . '`, ' . $next_id . ')">></a></li>
                ';
            }
        } else {
            if ($page_array[$count] == '...') {
                ...
                $page_link .= '
                <li class="page-item">
                    <a class="page-link" href="javascript:load_data_deny(`' . $_POST["query"] . '`, ' . $page_array[$count] . ')">' . $page_array[$count] . '</a>
                </li>
                ';
            }
        }
